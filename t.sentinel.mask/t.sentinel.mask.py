...
def main():
    ...
    if not grass.find_program("i.sentinel.mask.worker", "--help"):
        grass.fatal(
            _(
                "The 'i.sentinel.mask.worker' module was not found, install it first:"
            )
            + "\n"
    ...
    for strdsrast, time in zip(strdsrasters, times):
        ...
        if stats["min"] == "NULL" and stats["max"] == "NULL":
            grass.warning(
                _(
                    "Raster {} only consists of NULL() in current "
    ...
    for s2_scene_name in s2_scenes:
        ...
        newmapset = s2_scene["clouds"]
        if grass.find_file(
            s2_scene["clouds"], element="raster", mapset=newmapset
        )["file"]:
            if options["min_size_clouds"]:
                try:
                    ...
                    grass.message(
                        _(
                            "No clouds larger than %s ha detected. Image is considered cloudfree."
                            % options["min_size_clouds"]
                        )
                    )
                    exp_null = "%s = null()" % s2_scene["clouds"]
                    grass.run_command(
                        "r.mapcalc", expression=exp_null, quiet=True
                    )
            else:
                grass.run_command(
                    "g.copy",
            ...
            grass.run_command(
                "r.mapcalc", expression="%s = null()" % s2_scene["clouds"]
            )
        if options["output_shadows"]:
            if grass.find_file(
                s2_scene["shadows"], element="raster", mapset=newmapset
            )["file"]:
                if options["min_size_shadows"]:
                    try:
                        ...
                        grass.message(
                            _(
                                "No shadows larger than %s ha detected. Image is considered shadowfree."
                ...
                grass.run_command(
                    "r.mapcalc", expression="%s = null()" % s2_scene["shadows"]
                )
    ...
    file.close()
    grass.run_command(
        "t.register", input=strdsout, file=registerfile, quiet=True
    )
    # remove registerfile
