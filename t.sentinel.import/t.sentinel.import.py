...
try:
    ...
    grass.warning(
        "You need to install psutil to use this module: " "pip install psutil"
    )
    ...
    if nprocs > nprocs_real:
        grass.warning(
            _(
                ...
                "Setting nprocs to {}."
            ).format(nprocs, nprocs_real, nprocs_real)
        )
...
def main():
    ...
    if not grass.find_program("i.sentinel.download", "--help"):
        grass.fatal(
            _(
                "The 'i.sentinel.download' module was not found, install it first:"
            )
            + "\n"
    ...
    if not grass.find_program("i.sentinel.import", "--help"):
        grass.fatal(
            _(
                "The 'i.sentinel.import' module was not found, install it first:"
            )
            + "\n"
            + "g.extension i.sentinel"
        )
    if not grass.find_program("i.sentinel.parallel.download", "--help"):
        grass.fatal(
            _(
                "The 'i.sentinel.parallel.download' module was not found, install it first:"
            )
            + "\n"
            + "g.extension i.sentinel"
        )
    ...
    if not grass.find_program("r.mapcalc.tiled", "--help"):
        grass.fatal(
            _("The 'r.mapcalc.tiled' module was not found, install it first:")
            + "\n"
    ...
    try:
        download_dir = os.path.join(
            tmpdirectory, "download_{}".format(os.getpid())
        )
    ...
    if flags["a"]:
        sen2cor_folder = os.path.join(
            tmpdirectory, "sen2cor_{}".format(os.getpid())
        )
        try:
        ...
        if os.path.exists(os.path.join(download_dir, subfolder)):
            ...
            if single_folders is True:
                ...
                if subfolder.endswith(".SAFE"):
                    ...
                    grass.warning(
                        _(
                            "{} is not in .SAFE or .zip format, " "skipping..."
                        ).format(os.path.join(download_dir, subfolder))
                    )
                    continue
                import_kwargs["pattern_file"] = pattern_file
            import_kwargs["input"] = directory
            i_sentinel_import = Module(
                "i.sentinel.import.worker", run_=False, **import_kwargs
            )
    ...
    if cur_mapset != start_cur_mapset:
        grass.fatal(
            "New mapset is <%s>, but should be <%s>"
    ...
    for new_mapset in mapsetids:
        for vect in grass.parse_command(
            "g.list", type="vector", mapset=new_mapset
        ):
            cloudlist.append(vect)
            grass.run_command(
                "g.copy", vector=vect + "@" + new_mapset + "," + vect
            )
        for rast in grass.parse_command(
            "g.list", type="raster", mapset=new_mapset
        ):
            grass.run_command(
                "g.copy", raster=rast + "@" + new_mapset + "," + rast
            )
            if "CLOUDS" in rast:
                ...
                if options["offset"]:
                    ...
                    tmp_desc_dir = os.path.join(
                        tmpdirectory, "descriptions_json"
                    )
                    if not os.path.isdir(tmp_desc_dir):
                        try:
                            ...
                            grass.fatal(
                                _(f"Unable to create directory {tmp_desc_dir}")
                            )

                    desc_file_save = os.path.join(
                        tmp_desc_dir, f"{rast}_description.json"
                    )
                    desc_file_in = os.path.join(
                        json_standard_folder, rast, "description.json"
                    )
                    ...
                    mapc_exp = (
                        f"{tmp_rast} = if({rast} + {options['offset']} < 0, "
    ...
    if options["strds_output"]:
        ...
        file.close()
        grass.run_command(
            "t.register", input=strds, file=registerfile, quiet=True
        )
        # remove registerfile
        ...
        if flags["c"]:
            ...
            grass.run_command(
                "t.create",
                output=stdsclouds,
                title="Sentinel-2_sen2cor_clouds",
                desc="Sentinel-2_sen2cor_clouds",
                quiet=True,
                type=stds_type,
            )
