...
def main():
    ...
    if options["region"]:
        grass.run_command(
            "g.region", region=options["region"] + "@" + old_mapset
        )
    ...
    for key, val in kwargs.items():
        if not key == "flags":
            ...
            flagstr += val
    cmd = grass.Popen(
        "i.sentinel.import --q %s -%s" % (kwargsstr, flagstr),
        shell=True,
    ...
    for resp_line in resp:
        if (
            ...
            grass.warning(
                _(
                    "Input raster <%s> does not overlap current computational region"
                )
                % raster_var
            )
    ...
    if flags["n"]:
        rasters = list(
            grass.parse_command("g.list", type="raster", mapset=".").keys()
        )
    ...
    if flags["i"]:
        ...
        raster_resamp_list = list(
            grass.parse_command(
                "g.list", type="raster", pattern="*B*_10m"
            ).keys()
        )
        list_20m = list(
            grass.parse_command(
                "g.list", type="raster", pattern="*B*_20m"
            ).keys()
        )
        list_60m = list(
            grass.parse_command(
                "g.list", type="raster", pattern="*B*_60m"
            ).keys()
        )
        ...
        if len(raster_resamp_list) > 0:
            for raster in raster_resamp_list:
                ...
                if raster.endswith("10m"):
                    grass.run_command(
                        "g.rename", raster="%s,%sTMP" % (raster, raster)
                    )
                    raster = "%sTMP" % (raster)
